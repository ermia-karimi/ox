...
const inhaleTime = 2000; // دم = ۴ ثانیه
...
const button = document.getElementById('startBtn2');

button.addEventListener('click', () => {
    const today = new Date().toISOString().split('T')[0]; // YYYY-MM-DD
    const clicks = JSON.parse(localStorage.getItem('clickData2')) || {};
    clicks[today] = (clicks[today] || 0) + 1; // فقط یک واحد اضافه شود
    localStorage.setItem('clickData2', JSON.stringify(clicks));
});
