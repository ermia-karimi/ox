...
const startBtn1 = document.getElementById('startBtn5');
const statusText = document.getElementById('circle2');

const inhaleTime = 4000; // دم = ۴ ثانیه
const exhaleTime = 8000; // بازدم = ۶ ثانیه
...
const button = document.getElementById('startBtn5');

button.addEventListener('click', () => {
    const today = new Date().toISOString().split('T')[0]; // YYYY-MM-DD
    const clicks = JSON.parse(localStorage.getItem('clickData5')) || {};
    clicks[today] = (clicks[today] || 0) + 1; // فقط یک واحد اضافه شود
    localStorage.setItem('clickData5', JSON.stringify(clicks));
});
